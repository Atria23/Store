...
class ProductController extends Controller
{
    ...
    public function showProducts($category, $brand)
    {
        // Ambil data brand berdasarkan nama
        $brandData = Brand::where('name', $brand)->firstOrFail();

        // Ambil semua type yang terkait dengan brand
        $types = Type::where('brand_id', $brandData->id)->get();

        // Ambil semua produk pertama kali (tanpa filter)
        $products = Barang::where('brand_id', $brandData->id)->get();

        return Inertia::render('User/ProductPage', [
            'category' => $category,
            'brand' => $brand,
            'types' => $types,
            'products' => $products, // Kirim semua produk dulu, nanti difilter di React
        ]);
    }
