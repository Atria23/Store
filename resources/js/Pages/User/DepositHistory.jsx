...
const DepositHistory = ({ deposits }) => {
  ...
  const [modalType, setModalType] = useState(""); // 'confirm' atau 'info'
  const [confirmId, setConfirmId] = useState(null);

  const handleConfirm = (id) => {
    setConfirmId(id);
    setModalMessage("Apakah kamu yakin ingin mengonfirmasi deposit ini?");
  ...
  const handleConfirmDeposit = () => {
    ...
    fetch(`/deposit/confirm/${confirmId}`, {
      ...
      .then((data) => {
        if (data.success) {
          window.location.reload(); // reload kalau sukses
        } else {
          setModalMessage("Konfirmasi gagal: tidak ditemukan data yang sesuai.");
          setModalType("info");
          setShowModal(true);
        }
      })
