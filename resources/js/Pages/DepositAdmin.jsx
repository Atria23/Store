...
export default function DepositAdmin() {
    ...
    return (
        <div className="max-w-[500px] mx-auto">
            {/* Header */}
            <div className="w-full flex flex-row space-x-4 items-center px-4 py-2 bg-main">
                <button
                    className="shrink-0 w-6 h-6"
                    onClick={() => window.history.back()}
                >
            ...
            {result && result.type === 'success' && (
                <div className="mt-6 p-4 bg-green-100 border rounded text-green-800 space-y-3">
                    ...
                    <InfoRow
                        label="Jumlah yang harus ditransfer"
                        value={Number(result.amount).toLocaleString('id-ID')}
                        onCopy={() => copyToClipboard(result.amount.toString())}
                    />

                    <InfoRow
                        label="Berita transfer"
                        value={result.notes}
                        onCopy={() => copyToClipboard(result.notes)}
                    />

                    <InfoRow
                        label="Nomor Rekening"
                        value={result.account_number}
                        onCopy={() => copyToClipboard(result.account_number)}
                    />
