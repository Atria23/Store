...
//     // State untuk alur Internet Massal
...
//     // CSRF token: Inertia/Axios usually handle this automatically in Laravel,
//     // but including for fetch API if needed explicitly, though generally not required.
...
//         try {
//             const response = await fetch(route('pascainternet.inquiry'), {
//                 method: 'POST',
...
//         try {
//             const response = await fetch(route('pascainternet.bulk-inquiry'), {
//                 method: 'POST',
//                 headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': csrfToken },
//                 body: JSON.stringify({
...
//                 paymentResponse = await fetch(route('pascainternet.bulk-payment'), {
//                     method: 'POST',
//                     headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': csrfToken },
//                     body: JSON.stringify({ customer_nos_to_pay: customerNosToPay }),
//                 });
//             } else {
//                 paymentResponse = await fetch(route('pascainternet.payment'), {
//                     method: 'POST',
//                     headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': csrfToken },
...
//     return (
//         <>
//             <Head title="Internet Pascabayar" />
...
//                             </svg>
//                         </button>
//                         <div className="font-semibold text-white text-lg">Tagihan Internet</div>
//                     </div>
...
//                                     placeholder="Cari provider Internet..."
//                                     value={searchTerm}
...
//                                         {isBulkMode ? 'Pembayaran Tagihan Internet Massal' : 'Pembayaran Tagihan Internet Anda berhasil.'}
//                                     </p>
//                                 </div>
...
//                                                         </div>
//                                                         {parseFloat(detail.admin) > 0 && ( // Admin per detail for internet
//                                                             <div className="flex justify-between pl-2">
...
//                                                     const totalAdminFromDetails = item.admin;
//                                                     return (
...
export default function Internet({ auth, products }) {
    ...
    // NEW: RC to message mapping (keep this as is)
    const rcMessages = useMemo(() => ({
        ...
        // Custom RC codes from your backend (adjust if needed)
    ...
    // Helper function to get RC message (keep this as is)
    ...
    const handleBulkInquiry = async (e) => {
        e.preventDefault();
        if (!selectedProduct) {
            setError({ message: 'Silakan pilih produk Internet terlebih dahulu.', rc: null });
            return;
        }
        if (!selectedProduct.seller_product_status) {
        ...
        if (customerNos.length === 0) {
            setError({ message: 'Masukkan setidaknya satu ID Pelanggan yang valid.', rc: null });
            setIsLoading(false);
            return;
        }
                                                ...
                                                {bulkInquiryResults.failed.map((item, index) => (
                                                    <div key={index} className="border border-red-300 bg-red-50 p-3 rounded-lg space-y-1 text-sm">
                                                        ...
                                                        <p className="text-xs text-red-600 mt-1">
                                                            ...
                                                            {(!getRcDescription(item.rc) && item.rc) ? ` (RC: ${item.rc})` : ''} {/* MODIFIED: Error display logic */}
                                                        </p>
                                                    </div>
                                                ))}
                                            </>
