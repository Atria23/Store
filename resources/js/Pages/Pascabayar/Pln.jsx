...
const getResponseMessage = (rc) => rcMessages[rc] || `Transaksi Gagal (RC: ${rc || 'Tidak Diketahui'})`;
...
export default function PlnPascaIndex({ auth }) {
    ...
    const [error, setError] = useState({ message: '', rc: '' });
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [password, setPassword] = useState('');
    const [showPassword, setShowPassword] = useState(false);
    ...
    const handleInquiry = async (e) => {
        ...
        try {
            ...
            const data = await response.json(); // Data ini akan selalu memiliki 'rc' dan 'message' dari controller
            if (!response.ok) {
                // Backend sudah mengirimkan message yang sesuai dengan RC, jadi kita tampilkan itu.
                setError({
                    message: data.message || getResponseMessage(data.rc || '02'),
                    rc: data.rc || '02'
                });
                return;
            }
    ...
    const handleBulkInquiry = async (e) => {
        ...
        if (customerNos.length === 0) {
            setError({ message: getResponseMessage('02'), rc: '02' }); // RC '02' untuk kegagalan internal
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            const data = await response.json(); // Data ini akan selalu memiliki 'rc' dan 'message' dari controller jika gagal
            if (!response.ok) {
                setError({
                    message: data.message || getResponseMessage(data.rc || '02'),
                    rc: data.rc || '02'
    ...
    const handleBayarClick = () => {
        ...
        if (userBalance < amountToCheck) {
            setError({
                message: getResponseMessage('02'), // RC '02' untuk saldo tidak cukup
                rc: '02'
            });
            return;
        }
    ...
    const handleSubmitPayment = async (e) => {
        ...
        try {
            ...
            const verifyData = await verifyResponse.json();
            if (!verifyResponse.ok || !verifyData.success) {
                setError({ message: verifyData.message || 'Password yang Anda masukkan salah.', rc: '02' });
                return;
            }
            ...
            const data = await paymentResponse.json(); // Data ini akan selalu memiliki 'rc' dan 'message' dari controller jika gagal
            if (!paymentResponse.ok) {
                setError({
                    message: data.message || getResponseMessage(data.rc || '02'), // Gunakan pesan dari RC_MESSAGES
                    rc: data.rc || '02'
                });
                isBulkMode ? setBulkPaymentResults(data) : setPaymentResult(data);
                return;
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm border">
                        {isViewingPaymentResult ? (
                                            ...
                                            {bulkPaymentResults.results.map((item, index) => (
                                                <li key={index} className={`flex justify-between items-center py-1 ${item.status === 'Gagal' ? 'text-red-700' : ''}`}>
                                                    ...
                                                    <span className="text-right">
                                                        ...
                                                        {item.status === 'Gagal' && ` - ${getResponseMessage(item.rc || '02')}`}
                                                    </span>
                                                </li>
                                            ))}
                                        </ul>
                                            ...
                                            <div className="space-y-2 text-sm border-t pt-2">
                                                ...
                                                {inquiryResult.desc.detail.map((detail, index) => (
                                                    <div key={index} className="border-b pb-2 mb-2 last:border-b-0 last:pb-0">
                                                        ...
                                                        {parseFloat(detail.denda ?? 0) > 0 && ( // Ensure denda is parsed as float
                                                            <div className="flex justify-between pl-2">
                                                                <span className="text-gray-500">Denda (per periode)</span>
                                                                <span className="font-medium text-red-600">{formatRupiah(detail.denda)}</span>
                                                            </div>
                            ...
                            <div className="space-y-6">
                                ...
                                {error.message && !isModalOpen && <p className="text-red-500 text-xs text-center pt-2">{error.message} (RC: {error.rc})</p>}
                            </div>
                        )}
                    </div>
                </main>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
                        <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-sm space-y-4">
                            ...
                            <form onSubmit={handleSubmitPayment}>
                                ...
                                {error.message && <p className="text-red-500 text-xs text-center pt-2">{error.message} (RC: {error.rc})</p>}
                                <div className="flex justify-end space-x-2 pt-4">
                                    <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg">Batal</button>
                                    <button type="submit" disabled={isLoading} className="px-4 py-2 text-sm font-medium text-white bg-main hover:bg-blue-700 rounded-lg disabled:bg-gray-400">
                                        {isLoading ? "Memproses..." : "Konfirmasi"}
