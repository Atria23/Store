...
export default function Pbb({ auth, products }) {
    ...
    const rcMessages = useMemo(() => ({
        ...
        "PRODUCT_NOT_ACTIVE": "Produk PBB tidak aktif.",
        "API_CONNECTION_FAILED": "Gagal terhubung ke server penyedia layanan PBB.",
        "UNKNOWN_ERROR": "Terjadi kesalahan tidak dikenal saat inquiry PBB."
    }), []);
    ...
    const handleBulkInquiry = async (e) => {
        e.preventDefault();
        if (!selectedProduct) {
            setError({ message: 'Silakan pilih produk PBB terlebih dahulu.', rc: null });
            return;
        }
        if (!selectedProduct.seller_product_status) {
        ...
        if (customerNos.length === 0) {
            setError({ message: 'Masukkan setidaknya satu ID Pelanggan (NOP) yang valid.', rc: null });
            setIsLoading(false);
            return;
        }
