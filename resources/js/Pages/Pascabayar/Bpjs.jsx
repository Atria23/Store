...
export default function BpjsPascaIndex({ auth }) {
    ...
    const rcMessages = useMemo(() => ({
        ...
        "14": "Saldo provider tidak mencukupi.", // Contoh RC dari dummy payment
        "40": "Payload Error",
        "50": "Transaksi Tidak Ditemukan", // ID Pelanggan tidak ditemukan
        ...
        // Custom RC codes from your backend
        "NO_PRODUCT_ACTIVE": "Tidak ada produk BPJS Kesehatan yang aktif.",
        ...
        "API_CONNECTION_FAILED_PAYMENT": "Gagal terhubung ke server penyedia layanan saat pembayaran.",
        "API_RESPONSE_INCOMPLETE": "Respon API provider tidak lengkap.",
    ...
    const handleInquiry = async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                setError({
                    message: data.message || 'Gagal melakukan pengecekan tagihan BPJS.',
                    rc: data.rc || null
                });
                setIsLoading(false);
                return;
    ...
    const handleBulkInquiry = async (e) => {
        ...
        if (customerNos.length === 0) {
            setError({ message: 'Masukkan setidaknya satu ID Pelanggan BPJS yang valid.', rc: null });
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                // MODIFIED: Set error state as an object
                setError({
                    message: data.message || 'Gagal melakukan pengecekan tagihan BPJS massal.',
                    rc: data.rc || null
                });
                setIsLoading(false);
                return;
    ...
    const handleBayarClick = () => {
        const amountToCheck = isBulkMode ? totalAmountToPayBulk : inquiryResult?.selling_price;
        if (userBalance < amountToCheck) {
            setError({ message: "Saldo tidak mencukupi untuk melakukan transaksi.", rc: null });
            return;
        }
        setError(null); // MODIFIED: Clear error by setting to null
        setPassword('');
    ...
    const handleSubmitPayment = async (e) => {
        ...
        if ((!inquiryResult && !isBulkMode) || (isBulkMode && (!bulkInquiryResults || bulkInquiryResults.successful.length === 0))) {
            setError({ message: 'Tidak ada tagihan BPJS yang siap dibayar.', rc: null });
            return;
        }
        ...
        try {
            ...
            if (!paymentResponse.ok) {
                // MODIFIED: Set error state as an object
                setError({
                    message: data.message || 'Gagal melakukan pembayaran BPJS.',
                    rc: data.rc || null
                });
                isBulkMode ? setBulkPaymentResults(data) : setPaymentResult(data);
                setIsLoading(false);
                                ...
                                {inquiryResult && ( // Single inquiry result display
                                    <>
                                        ...
                                        <div className="space-y-2 text-sm border-t pt-2">
                                            <h4 className="font-semibold text-md text-gray-800 pb-1">Ringkasan Pembayaran</h4>
                                            <div className="flex justify-between">
                                                <span className="text-gray-500">Total Nilai Tagihan Pokok</span>
                                                <span className="font-medium text-gray-900">{formatRupiah(inquiryResult.price)}</span>
                                            </div>
                                            {inquiryResult.denda > 0 && (
                                                <div className="flex justify-between">
