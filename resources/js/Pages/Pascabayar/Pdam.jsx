...
//                                                             <span className="font-medium">{formatRupiah((detail.nilai_tagihan ?? 0) - (detail.denda ?? 0))}</span> {/* Nilai tagihan pokok per periode */}
//                                                         </div>
...
//                                                             </div>
//                                                         )}
//                                                         {parseFloat(detail.biaya_lain ?? 0) > 0 && (
//                                                             <div className="flex justify-between pl-2">
...
export default function Pdam({ auth, products }) {
    ...
    const rcMessages = useMemo(() => ({
        ...
        "PRODUCT_NOT_ACTIVE": "Produk PDAM tidak aktif.",
        "API_CONNECTION_FAILED": "Gagal terhubung ke server penyedia layanan PDAM.",
        "UNKNOWN_ERROR": "Terjadi kesalahan tidak dikenal saat inquiry PDAM."
    }), []);
    ...
    const handleBulkInquiry = async (e) => {
        e.preventDefault();
        if (!selectedProduct) {
            setError({ message: 'Silakan pilih produk PDAM terlebih dahulu.', rc: null });
            return;
        }
        if (!selectedProduct.seller_product_status) {
