...
class PascaBpjsController extends Controller
{
    ...
    private function _performSingleInquiry(string $customerNo): array // MODIFIED: Always return array
    {
        ...
        if ($availableProducts->isEmpty()) {
            ...
            return [
                'success' => false,
                'message' => 'Tidak ada produk BPJS Kesehatan yang aktif ditemukan.',
                'rc' => 'NO_PRODUCT_ACTIVE' // Custom RC
            ];
        }
        ...
        if (!$productToUseForCommission) {
            ...
            return [
                'success' => false,
                'message' => 'Tidak ada produk BPJS Kesehatan yang aktif untuk perhitungan komisi.',
                'rc' => 'NO_PRODUCT_FOR_COMMISSION' // Custom RC
            ];
        }
        ...
        if (env('APP_ENV') === 'local') {
            ...
            if ($customerNo === '9999999999999') { // Simulate customer not found
                return [
                    'success' => false,
                    'message' => 'ID Pelanggan BPJS tidak ditemukan atau tidak valid (DUMMY).',
                    'rc' => '50' // Contoh RC: Transaksi Tidak Ditemukan
                ];
            }
            if ($customerNo === '8888888888888') { // Simulate product gangguan
                return [
                    'success' => false,
                    'message' => 'Produk BPJS mengalami gangguan (DUMMY).',
                    'rc' => '55' // Contoh RC: Produk Gangguan
                ];
            }
            if ($customerNo === '7777777777777') { // Simulate tagihan sudah lunas
                return [
                    'success' => false,
                    'message' => 'Tagihan BPJS untuk ID ini sudah lunas (DUMMY).',
            ...
            $inquiryDataFromApi = [ // Use a temporary variable to build data
                ...
                'buyer_sku_code' => 'bpjs', // Generic SKU for BPJS
                'admin' => $dummyAdminField, // Admin dari provider
                'message' => 'INQ BPJS Sukses (DUMMY).', // MODIFIED: Always include message
                'status' => 'Sukses',
            ...
            Log::info("Inquiry BPJS (DUMMY) Sukses untuk Customer No: {$customerNo}", ['customer_no' => $customerNo, 'dummy_response' => $inquiryDataFromApi]);
            ...
            if (!isset($responseData['data']) || $responseData['data']['status'] !== 'Sukses') {
                $errorMessage = $responseData['data']['message'] ?? 'Provider sedang sibuk atau data tidak valid.';
                $errorCode = $responseData['data']['rc'] ?? 'PROVIDER_ERROR_UNKNOWN'; // NEW: Capture provider's RC
                Log::warning("Inquiry BPJS Gagal dari Provider. Customer No: {$customerNo}. Pesan: {$errorMessage} (RC: {$errorCode})", [
                    'customer_no' => $customerNo, 'ref_id' => $ref_id, 'api_response' => $responseData,
                ]);
        ...
        // --- COMMON PROCESSING LOGIC (for both real and dummy data) ---
        // (inquiryDataFromApi already contains data from dummy or real API)
        ...
        // Calculate our final selling price to the user
        // This directly implements the formula: (total bill amount + total admin fee) - discount
        $finalSellingPrice = $totalAmountBeforeOurDiskon - $finalDiskon;
        ...
        $successfulInquiryData['price'] = $pureBillPriceFromApi; // This is the "pure bill" amount from API
        $successfulInquiryData['admin'] = $adminFeeFromApi;         // This is the provider's admin fee
        $successfulInquiryData['diskon'] = $finalDiskon;
        $successfulInquiryData['jumlah_lembar_tagihan'] = $jumlahLembarTagihan;
        $successfulInquiryData['selling_price'] = $finalSellingPrice; // This is what the user will pay
        $successfulInquiryData['buyer_sku_code'] = $productToUseForCommission->buyer_sku_code; // Use actual SKU from product
    ...
    /**
     * Menerima request untuk pengecekan tagihan BPJS Kesehatan.
     *
    ...
    public function inquiry(Request $request)
    {
        ...
        $inquiryResult = $this->_performSingleInquiry($customerNo); // MODIFIED: Use $inquiryResult

        // MODIFIED: Check the 'success' key from the returned array
        if ($inquiryResult['success']) {
            session(['postpaid_bpjs_inquiry_data' => $inquiryResult]); // Use a distinct session key
            return response()->json($inquiryResult);
        } else {
            // MODIFIED: Directly use message and RC from the returned inquiryResult array
            return response()->json([
                'message' => $inquiryResult['message'] ?? 'Gagal melakukan pengecekan tagihan BPJS. Silakan coba lagi nanti atau periksa nomor pelanggan.', // Fallback message
