...
class PascaInternetController extends Controller
{
     ...
     * @return array|null Returns processed inquiry data on success, null on failure.
     */
    private function _performSingleInternetInquiry(string $customerNo, PostpaidProduct $product): array // MODIFIED: Always return array
    {
        $current_sku = $product->buyer_sku_code;
        $ref_id = 'internet-' . substr(str_replace('-', '', Str::uuid()->toString()), 0, 15);
        $username = env('P_U');
        ...
        if (env('APP_ENV') === 'local') {
            ...
            if ($customerNo === '9999') { // Simulate customer not found
                return [
                    'success' => false,
                    'message' => 'ID Pelanggan Internet tidak ditemukan (DUMMY).',
            ...
            if ($customerNo === '8888') { // Simulate product gangguan
                return [
                    'success' => false,
                    'message' => 'Produk Internet mengalami gangguan (DUMMY).',
            ...
            if ($customerNo === '7777') { // Simulate tagihan sudah lunas
                return [
                    'success' => false,
                    'message' => 'Tagihan Internet untuk ID ini sudah lunas (DUMMY).',
            ...
            $inquiryDataFromApi = [ // Use a temporary variable to build data
                'status' => 'Sukses',
                'message' => 'Inquiry Internet dummy berhasil.', // MODIFIED: Always include message
                'customer_name' => $dummyCustomerName,
                'customer_no' => $customerNo,
                'buyer_sku_code' => $current_sku,
                'price' => $dummyPriceField,
            ...
            Log::info('Internet Inquiry API Response (DUMMY):', ['response_data' => $inquiryDataFromApi, 'customer_no' => $customerNo, 'sku' => $current_sku, 'ref_id' => $ref_id]);
        } else {
            // --- END DUMMY DATA ---
            ...
            if (!$product->seller_product_status) {
                ...
                return [
                    'success' => false,
                    'message' => 'Produk Internet tidak aktif.',
            ...
            try {
                ...
                if (!isset($responseData['data']) || $responseData['data']['status'] !== 'Sukses') {
                    ...
                    Log::warning("Inquiry Internet Gagal dari Provider untuk SKU: {$current_sku}. Pesan: {$errorMessage} (RC: {$errorCode})", ['response' => $responseData, 'customer_no' => $customerNo]);
    ...
    public function inquiry(Request $request)
    {
        ...
        if (!$product || !$product->seller_product_status) {
            return response()->json([
                'message' => 'Produk Internet tidak tersedia atau tidak aktif.',
                'rc' => 'PRODUCT_NOT_ACTIVE' // Custom RC
            ], 503);
        }

        $inquiryResult = $this->_performSingleInternetInquiry($customerNo, $product); // MODIFIED: Use $inquiryResult
