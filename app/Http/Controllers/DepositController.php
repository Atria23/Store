...
class DepositController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //     if ($response->successful()) {
        //         $digiflazzBalance = (int) ($response['data']['deposit'] ?? 0);
        //         $userTotalBalance = DB::table('users')->sum(DB::raw('balance + points'));
        //         $threshold = $validated['amount'] + $userTotalBalance + 500000;
        
        //         if ($threshold > $digiflazzBalance) {
        //             $nominal = $validated['amount'] < 200000 ? 200000 : $validated['amount'];
        //             $fallbacks = [
        //                 [
        //                     'account' => '6042888890',
        //                     'notes' => 'Deposit fallback untuk userA',
        //                     'bank' => 'BCA',
        //                 ],
        //                 [
        //                     'account' => '213501000291307',
        //                     'notes' => 'Deposit fallback untuk userB',
        //                     'bank' => 'BRI',
        //                 ],
        //             ];
        
        //             $emails = config('custom.admin_deposit_emails');
        ...
        //             \Log::info('Fallback email deposit admin telah dikirim.');
        //         }
        //     }
        // } catch (\Exception $e) {
        //     \Log::error('Gagal melakukan pengecekan saldo Digiflazz atau mengirim email fallback: ' . $e->getMessage());
        // }
        try {
            $username = env('P_U');
            $apiKey = env('P_AK');
            ...
            $responseSaldo = Http::post('https://api.digiflazz.com/v1/cek-saldo', [
                'cmd' => 'deposit',
                'username' => $username,
                'sign' => $sign,
            ]);
            ...
            if ($responseSaldo->successful()) {
                $digiflazzBalance = (int) ($responseSaldo['data']['deposit'] ?? 0);
                $userTotalBalance = DB::table('users')->sum(DB::raw('balance + points'));
                $threshold = $validated['amount'] + $userTotalBalance + 500000;
                ...
                if ($threshold > $digiflazzBalance) {
                    ...
                    foreach ($fallbacks as $f) {
                        $depositResponse = Http::post('https://api.digiflazz.com/v1/deposit', [
                            'username' => $username,
                            'amount' => $nominal,
                            'bank' => $f['bank'],
                        ...
                        if ($depositResponse->successful() && $depositResponse->json('data.rc') === '00') {
                            $amount = $depositResponse->json('data.amount');
                            $notes = $depositResponse->json('data.notes');
                            ...
                            \Log::error('Gagal request deposit admin fallback untuk ' . $f['owner_name'], [
                                'response' => $depositResponse->json()
                            ]);
                        }
                    }
                }
