...
class PascaPbbController extends Controller
{
     ...
     * @return array|null Returns processed inquiry data on success, null on failure.
     */
    private function _performSinglePbbInquiry(string $customerNo, PostpaidProduct $product): array // MODIFIED: Always return array
    {
        $current_sku = $product->buyer_sku_code;
        $ref_id = 'pbb-' . substr(str_replace('-', '', Str::uuid()->toString()), 0, 15);
        $username = env('P_U');
        ...
        if (env('APP_ENV') === 'local') {
            ...
            if ($customerNo === '9999') { // Simulate customer not found
                return [
                    'success' => false,
                    'message' => 'NOP (Nomor Objek Pajak) tidak ditemukan (DUMMY).',
            ...
            if ($customerNo === '8888') { // Simulate product gangguan
                return [
                    'success' => false,
                    'message' => 'Produk PBB mengalami gangguan (DUMMY).',
            ...
            if ($customerNo === '7777') { // Simulate tagihan sudah lunas
                return [
                    'success' => false,
                    'message' => 'Tagihan PBB untuk NOP ini sudah lunas (DUMMY).',
            ...
            $dummyPriceField = $totalPureBillAmountDummy + $totalDendaAmountDummy;
            $dummyAdminField = $adminFeePerTransaction;
            ...
            $inquiryDataFromApi = [ // Use a temporary variable to build data
                'status' => 'Sukses',
                'message' => 'Inquiry PBB dummy berhasil.', // MODIFIED: Always include message
                'customer_name' => $dummyCustomerName,
                'customer_no' => $customerNo,
                'buyer_sku_code' => $current_sku,
            ...
            Log::info('PBB Inquiry API Response (DUMMY):', ['response_data' => $inquiryDataFromApi, 'customer_no' => $customerNo, 'sku' => $current_sku, 'ref_id' => $ref_id]);
        } else {
            // --- END DUMMY DATA ---
            ...
            if (!$product->seller_product_status) {
                ...
                return [
                    'success' => false,
                    'message' => 'Produk PBB tidak aktif.',
            ...
            try {
                ...
                if (!isset($responseData['data']) || $responseData['data']['status'] !== 'Sukses') {
                    ...
                    Log::warning("Inquiry PBB Gagal dari Provider untuk SKU: {$current_sku}. Pesan: {$errorMessage} (RC: {$errorCode})", ['response' => $responseData, 'customer_no' => $customerNo]);
        ...
        $providerOriginalSellingPrice = (float) ($inquiryDataFromApi['selling_price'] ?? 0);
        $apiPriceComponentFromApi     = (float) ($inquiryDataFromApi['price'] ?? 0);
        ...
        if ($apiPriceComponentFromApi > $finalSellingPrice && $providerOriginalSellingPrice > 0) {
            Log::info("PBB Inquiry: finalSellingPrice overridden by provider_original_selling_price based on API 'price' component.", [
                'calculated_finalSellingPrice' => $finalSellingPrice,
                'apiPriceComponentFromApi' => $apiPriceComponentFromApi,
...
public function inquiry(Request $request)
    {
        ...
        if (!$product || !$product->seller_product_status) {
            ...
            return response()->json([
                'message' => 'Produk PBB tidak tersedia atau tidak aktif.',
                'rc' => 'PRODUCT_NOT_ACTIVE' // Custom RC
            ], 503);
        }

        $inquiryResult = $this->_performSinglePbbInquiry($customerNo, $product); // MODIFIED: Use $inquiryResult
